...
def run():
    ...
    for w_idx, week_date in enumerate(bt_weeks):
        ...
        if w_idx > 0:
            ...
            # S6 PnL realization with v3.6 STOP-LOSS SIMULATION
            # Stop: 8% below entry_price (≈ 1.5×ATR for typical AI stocks).
            # If close[di] drops below stop_price, realize loss at stop, mark for exit.
            STOP_PCT = 0.08
            basket6 = holdings["S6"]
            if not basket6:
                ...
                for h in basket6:
                    ...
                    if di < len(cl) and not math.isnan(cl[di]):
                        ...
                        stop_price = h["entry_price"] * (1 - STOP_PCT)
                        if cl[di] <= stop_price:
                            # STOPPED OUT: realize loss at stop (better than weekly close)
                            weighted_ret += (stop_price / base_price - 1) * h["weight"]
                            h["_stopped"] = True
                            h["last_price"] = stop_price
                            # Pay txn cost on exit
                            txn_weight += h["weight"]
                        else:
        ...
        # v3.6: drop any position that hit stop-loss this week (no min_hold override on stops)
        prev_basket = [h for h in holdings.get("S6", []) if not h.get("_stopped")]
        kept = []
        layer_count_s6 = {}
