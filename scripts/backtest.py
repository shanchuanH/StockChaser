"""Backtest 4 strategies on the AI-chain universe.

Strategies:
  S1  Buy & Hold SPY                          (baseline)
  S2  6-month avg monthly momentum, Top 3     (the article)
  S3  Composite score Top 10 (no gates)       (current ranking)
  S4  Three-Gate + ATR sizing  ★ our strategy
...
def run():
    ...
    eq = {"S1": [1.0], "S2": [1.0], "S3": [1.0], "S4": [1.0], "SPY": [1.0]}
    holdings = {"S2": [], "S3": [], "S4": []}  # current basket
    last_positions = {"S2": [], "S3": [], "S4": []}  # for display
    ...
    for w_idx, week_date in enumerate(bt_weeks):
        ...
        # Holdings are basket of tickers at last week's close → realize at this week's close
        if w_idx > 0:
            for s in ["S2", "S3", "S4"]:
                basket = holdings[s]
                if not basket:
                ...
                # Apply round-trip cost since we'll rebalance
        ...
        # Save snapshot of last week's positions for display
        if w_idx == len(bt_weeks) - 1:
            for s, basket in holdings.items():
                last_positions[s] = []
                for h in basket:
                    t = h["ticker"]
                    last_positions[s].append({
                        "ticker": t,
                        "layer": by_ticker[t]["layer"],
                        "score": round(feats[t]["comp"], 1),
                        "entry_price": round(h["entry_price"], 2),
                        "current_price": round(feats[t]["close"], 2),
                        "return": (feats[t]["close"] / h["entry_price"] - 1),
                    })

    # ----- Build output JSON matching the HTML schema -----
    # Use S4 as the "primary" strategy and SPY as benchmark
    summary = {
        **{f"{k}_metrics": equity_curve_metrics(eq[k], bt_weeks) for k in ["S1", "S2", "S3", "S4"]},
        "strategy_total_return": equity_curve_metrics(eq["S4"], bt_weeks)["total_return"],
        "spy_total_return": equity_curve_metrics(eq["S1"], bt_weeks)["total_return"],
        "alpha": equity_curve_metrics(eq["S4"], bt_weeks)["total_return"]
                  - equity_curve_metrics(eq["S1"], bt_weeks)["total_return"],
        "max_drawdown": equity_curve_metrics(eq["S4"], bt_weeks)["max_drawdown"],
        "sharpe_ratio": equity_curve_metrics(eq["S4"], bt_weeks)["sharpe"],
        "win_rate": equity_curve_metrics(eq["S4"], bt_weeks)["win_rate"],
    ...
    out = {
        ...
        "latest_positions": last_positions["S4"],
        "strategies": {
            "S1": "Buy & Hold SPY",
    ...
    # Print quick summary
    print(f"\nBacktest done. {len(bt_weeks)} weeks.")
    print(f"\n{'Strategy':<14} {'Total':>8} {'CAGR':>8} {'Sharpe':>7} {'MaxDD':>8} {'Win%':>6}")
    for s, label in [("S1", "B&H SPY"), ("S2", "6M-Mom Top3"), ("S3", "Comp Top10"), ("S4", "★ 3Gate+ATR")]:
        m = equity_curve_metrics(eq[s], bt_weeks)
        print(f"{label:<14} {m['total_return']*100:>7.2f}% {m['cagr']*100:>7.2f}% {m['sharpe']:>7.2f} {m['max_drawdown']*100:>7.2f}% {m['win_rate']*100:>5.1f}%")
